...
def create(embedding_dim=100,
    ...
    def calculate_sizes(inner_net_dims, n_classes = 0):
        ...
        #note that inner net dimensions is always a tuple, consisting of at least one hidden alyer and the number of classes
    ...
    # Define the hypernetwork using DRNN layers: input = (batch_size, timesteps, features)
    layers = [tf.keras.layers.InputLayer(input_shape=(None, embedding_dim * 2))]

    # return_sequences=False, when:your task involves transforming an entire sequence into a fixed-size representation (a vector)
    # We should only do this in the last layer, since all layers in between expect input = (batch_size, timesteps, features)
    try:
        layers += [tf.keras.layers.SimpleRNN(units=neurons, return_sequences=True)
                for neurons in hnet_hidden_dims[:-1]]
        # Indeed, the last layer does NOT return a sequence anymore
        layers += [tf.keras.layers.SimpleRNN(units=hnet_hidden_dims[-1], return_sequences=False)]
        
    except TypeError:
        layers += [tf.keras.layers.SimpleRNN(units=hnet_hidden_dims, return_sequences=False)]

    
    chunk_size = int(np.ceil(weight_num / n_chunks))
    weight_num = weight_num.numpy()

    # Set up the dilation rates manually
    # By using an exponential increase in the dilation rate, the receptive field of each layer grows exponentially, allowing the network to integrate information from a wider range of past time steps as the depth of the network increases
    for i, layer in enumerate(layers):
        if i == 0: #the first layer is the input layer
            continue
        else:
            layer.cell.dilation_rate = dilation_rate ** i  # Increase dilation rate exponentially with layer index



    if not depency_preservation_between_chunks:
        # Add the final dense layer with chunk_size output units
        layers.append(tf.keras.layers.Dense(chunk_size, activation='tanh',
                                     kernel_regularizer=tf.keras.regularizers.l2(l2reg)))
    else:
        # If dependency must be preserved within chunks, only one batch (of all timestemps) must generate all weights
        layers.append(tf.keras.layers.Dense(weight_num, activation='tanh',
                                     kernel_regularizer=tf.keras.regularizers.l2(l2reg)))
    

    
    hnet = tf.keras.Sequential(layers)
    hnet.build((None, embedding_dim * 2))
    ...
    class HNet(tf.keras.Model):
        ...
        def call(self, inputs, **kwargs):
            ...
            if added_classes:
                ...
                self.hnet.build((None, None ,embedding_dim * 2))
                ...
                if not depency_preservation_between_chunks:
                    ...
                    new_dense_layer = tf.keras.layers.Dense(self.weight_num, activation='tanh',
                                kernel_regularizer=tf.keras.regularizers.l2(l2reg))
                    new_weights = np.concatenate([last_layer_weights, np.random.randn(input_size_last_layer, 
                    ...
                    new_biases = np.reshape(new_biases, (1, len(new_biases)))
                    self.hnet.add(new_dense_layer)
                    self.hnet.layers[-1].set_weights([new_weights, np.squeeze(new_biases)])
...
if __name__ == "__main__":
    ...
    model = create(use_unique_task_embedding = use_unique_task_embedding, inner_net_dims = (200, 300, num_classes), convolution_layers=convolution_layers, hnet_hidden_dims = hnet_hidden_dims,
                    dilation_rate = dilation_rate, depency_preservation_between_chunks = depency_preservation_between_chunks,
                    final_soft_max_layer = final_soft_max_layer,
                    initialize_TE_with_zero_bias = initialize_TE_with_zero_bias, embedding_dim = embedding_dim)
    ...
    for trainable_ in model.trainable_weights:
        print(trainable_.shape)

...
"' The FULL model '"
def create_LSTM(embedding_dim=100,
           n_chunks=10,
    ...
    # Define the hypernetwork using LSTM layers: input = (batch_size, timesteps, features)
    layers = [tf.keras.layers.InputLayer(input_shape=(None, embedding_dim * 2))]

    try: 
        for neurons in hnet_hidden_dims:
            layers.append(LSTM(neurons, return_sequences=True))  # Return sequences to pass output to the next LSTM layer
    except TypeError:
        layers.append(LSTM(hnet_hidden_dims, return_sequences=True))

    if not depency_preservation_between_chunks:
        # Add the final LSTM/Dense layer with chunk_size output units. Unfortunately, there is not enough memory to add a final LSTM layer...
        if not final_LSTM_layer:
            layers.append(tf.keras.layers.Dense(chunk_size, activation='tanh', kernel_regularizer=tf.keras.regularizers.l2(l2reg)))
        else:
            layers.append(LSTM(chunk_size, activation='tanh', kernel_regularizer=tf.keras.regularizers.l2(l2reg)))
    else:
        ...
        if not final_LSTM_layer:
            ...
            layers.append(LSTM(weight_num, activation='tanh', kernel_regularizer=tf.keras.regularizers.l2(l2reg)))
    ...
    class HNet(tf.keras.Model):
        ...
        def call(self, inputs, **kwargs):
            ...
            if added_classes:
                ...
                if not depency_preservation_between_chunks:
                    if final_LSTM_layer:
                        # Add the final LSTM layer with chunk_size output units
                        new_dense_layer = LSTM(new_chunk_size, activation='tanh', kernel_regularizer=tf.keras.regularizers.l2(l2reg))

                        new_weights = np.concatenate([last_layer_weights, np.random.randn(input_size_last_layer, 
                                                                                    4*new_chunk_size-4*old_chunk_size)], axis=1)
                    

                        new_recurrent_kernel = np.pad(last_layer_recurrent_kernel, [(0, new_chunk_size - old_chunk_size), 
                                                (0, 4*new_chunk_size-4*old_chunk_size)],mode=random_pad)
                        
                        new_biases = np.concatenate([last_layer_biases, np.random.randn(4*new_chunk_size-4*old_chunk_size)], axis=0)
                    ...
                    if final_LSTM_layer:
                        # If dependency must be preserved within chunks, only one batch (of all timestemps) must generate all weights
                        new_dense_layer = LSTM(self.weight_num, activation='tanh', kernel_regularizer=tf.keras.regularizers.l2(l2reg))

                        #note that 4 times self.weights because LSTM has 4 sets of weights
                        new_weights = np.concatenate([last_layer_weights, np.random.randn(input_size_last_layer, 
                                                                                    4*self.weight_num-4*old_weight_num)], axis=1)
                        ...
                        new_recurrent_kernel = np.pad(last_layer_recurrent_kernel, [(0, self.weight_num - old_weight_num), 
                                                (0, 4*self.weight_num-4*old_weight_num)],mode=random_pad)


                        new_biases = np.concatenate([last_layer_biases, np.random.randn(4*self.weight_num-4*old_weight_num)], axis=0)
...
if __name__ == "__main__":
    ...
    if use_unique_task_embedding:
        model = create_LSTM(use_unique_task_embedding = use_unique_task_embedding, 
        ...
        model = create_LSTM(use_unique_task_embedding = use_unique_task_embedding, inner_net_dims = inner_net_dims, convolution_layers=convolution_layers,
                            depency_preservation_between_chunks = depency_preservation_between_chunks,
                            hnet_hidden_dims = hnet_hidden_dims,
                            n_chunks = n_chunks,
                            embedding_dim = embedding_dim,

...
def apply_gem_regularization(grads, memory_buffer, lrate, trainables, Average_GEM, margin):
    ...
    AGEM_result = [0 for grad in grads] #just like result, keep track of every single trainable parameter
        ...
        for old_gradients in memory_buffer:
            ...
            for count, (current_grad, old_grad) in enumerate(zip(grads, old_gradients)):
                ...
                if len(old_grad)> 1:
                    ...
                    GEM_result = tf.reduce_sum(tf.maximum(0, grad_sum))
                    ...
                    try:
                        if AGEM_result[count] == 0:
                            AGEM_result[count] += old_gradients[count]
                            A_result = tf.cast(GEM_result, tf.float32) * (old_grad - AGEM_result[count])
                    except:
                        AGEM_result[count] = (AGEM_result[count] + old_gradients[count])/2
                        A_result = tf.cast(GEM_result, tf.float32) * (old_grad - AGEM_result[count])

                    result[count] += A_result*grad_scale
...
def run(X_train = [np.random.rand(10, 128, 32, 1)]*3, 
            ...
            storage_capacity = 6,
            class_incremental_case = (True, 0.4, 3, (False, 0.1), (4, 0.5), 0.4, 6),
